...
Este *output* do R mostra os dados que serão utilizados para gerar o **variograma experimental** calculado para a variável `acumulado_24h` (chuva) em função das coordenadas espaciais `x` e `y`. A tabela apresenta, para cada distância média (`dist`, em metros), o número de pares de pontos considerados (`np`), a semivariância média (ou variabilidade média) (`gamma`), que mede o quão diferentes são os valores da variável entre esses pares, quanto maior o `gamma`, maior a dissimilaridade. Observa-se que de maneira geral `gamma` aumenta com a distância indicando perda de correlação espacial, mas com algumas oscilações possivelmente devido ao número reduzido de pares (`np`) ou à variabilidade amostral. Esse passo é fundamental para modelar a dependência espacial antes de aplicar métodos como a **krigagem**, que estima valores em locais não amostrados.
