...
- Ajustar modelos de regressão espacial (SEM e GWR).
:::

::: {.callout-note}
## Slides do Capítulo 4 - Dados de Área
...
- $y_i$: valor padronizado (ou original, dependendo da convenção) da variável na região $i$.
- $\bar{y}$: média da variável $z$.
...
Agora vamos converter para matriz de vizinhaça no formato tabela. 
...
```{r, echo=T}
# Converte uma lista de vizinhos espaciais (vizinhos) em linhas e depois transforma em um objeto sf (geometria de linhas).
viz.sf <- as(nb2lines(vizinhos_rainha, coords = coord), 'sf')

# Atribui ao objeto viz.sf o mesmo sistema de coordenadas (CRS) do mapa do Rio de Janeir
...
- `style = "W"`: define o estilo de normalização dos pesos espaciais (padronização row-standardized). Na realidade e como transformasse os pesos para que cada município tenha peso total = 1, dando importância igual a todos independente do número de vizinhos.
...
Além das análises exploratórias para dados espaciais de área, é possível avançar na modelagem estatística por meio de modelos que consideram explicitamente a dependência espacial presente nos dados. Entre os mais utilizados estão os modelos globais de autocorrelação espacial, que se dividem principalmente em três abordagens: **SAR** (*Spatial Autoregressive Model*), **SEM** (*Spatial Error Model*) e **CAR** (*Conditional Autoregressive Model*) (CRESSIE, 1993).
...
- O **modelo SAR** incorpora a dependência espacial diretamente na **variável dependente**, assumindo que o valor observado em uma região $y_i$ é influenciado pelos valores das regiões vizinhas $y_j$, gerando efeitos de transbordamento (*spillovers*). 

Esses efeitos de transbordamento podem ocorrer quando uma mudança em uma variável independente $X$ em uma localidade $i$ afeta a variável dependente $y$ não apenas na própria localidade $i$, mas também em **localidades vizinhas** $j \neq i$. Em outras palavras, o impacto de uma variável "extrapola" os limites da unidade geográfica original, transbordando para as unidades ao redor.

Sua especificação é:  

$$y_i = \rho \sum_{j=1}^{n} w_{ij} y_j + \beta_0 + \sum_{k=1}^{p} \beta_k x_{ik} + \varepsilon_i$$

::: box-dica4
Sendo:

- $y_i$: valor observado da variável dependente na unidade $i$;
- $\beta_0$: intercepto da regressão (constante);
- $\beta_k$: coeficiente da $k$-ésima variável explicativa;
- $x_{ik}$: valor da $k$-ésima variável explicativa para a unidade $i$;
- $\varepsilon_i$: erro aleatório independente e identicamente distribuído (i.i.d.), com $\varepsilon_i \sim N(0, \sigma^2)$;
- $\rho$ (rho): coeficiente de autocorrelação espacial da variável dependente (parâmetro a ser estimado). Se $|\rho| \leq 1$, temos a condição de estabilidade do processo espacial, garantindo que $(I - \rho W)$ seja inversível. O valor de $\rho$ indica a intensidade da influência dos valores vizinhos sobre o valor local;
- $w_{ij}$: elemento da matriz de pesos espaciais $W$, que define o grau de conexão entre as unidades $i$ e $j$. Por convenção, $w_{ii} = 0$ (nenhuma unidade é vizinha de si mesma).
:::

- O **modelo SEM** considera a dependência espacial na **estrutura dos erros**, sendo indicado quando a autocorrelação espacial residual provém de variáveis omitidas que são espacialmente correlacionadas. Sua especificação é:  
...
- $\lambda$: coeficiente de autocorrelação espacial dos erros (parâmetro a ser estimado);
...
Resumindo, podemos dizer então que se $|\lambda| \leq 1$, temos condição de estabilidade do processo espacial.
:::

- O **modelo CAR** também modela a dependência nos erros, mas por meio de uma especificação **condicional**: o valor do erro em uma região $i$ dado os valores dos vizinhos segue uma distribuição normal com média proporcional à média dos erros vizinhos. É muito utilizado em abordagens **bayesianas** para modelagem de risco em áreas geográficas (como epidemiologia e saúde pública). Sua especificação condicional é:  
$$y_i = X_i \beta + \varepsilon_i$$

$$\varepsilon_i \mid \varepsilon_j \sim N\left(\phi \sum_{j=1}^{n} w_{ij} \varepsilon_j,\ \tau_i^2\right)$$
::: box-dica4
Sendo:

- $y_i$: valor observado da variável dependente na unidade $i$;
- $X_i$: vetor linha de variáveis explicativas para a unidade $i$ (incluindo o intercepto);
- $\beta$: vetor de coeficientes de regressão (incluindo $\beta_0$ como intercepto);
- $\varepsilon_i$: erro da regressão (resíduo) na unidade $i$;
- $\varepsilon_i \mid \varepsilon_j$: distribuição condicional do erro na unidade $i$ **dados** os erros em todas as demais unidades $j \neq i$;
- $\sim$: "segue a distribuição" (lê-se: "é distribuído como");
- $N(\cdot, \cdot)$: distribuição Normal (Gaussiana), com primeiro argumento sendo a média e o segundo a variância;
- $\phi$ (*phi*): coeficiente de autocorrelação espacial condicional (parâmetro a ser estimado). Se $|\phi| \leq 1$, temos a condição de estabilidade do processo espacial, garantindo que a matriz de covariância $(I - \phi W)^{-1}$ seja positiva definida. $\phi$ mede a força da dependência entre o erro de uma unidade e a média dos erros de suas vizinhas;
:::

::: box-dica13
Enquanto os modelos SAR e SEM tratam a dependência de forma **simultânea** (o valor em cada local depende dos vizinhos e vice-versa), o modelo CAR adota uma perspectiva **condicional** (a distribuição de cada local é modelada a partir dos vizinhos). Essa distinção tem implicações importantes tanto na estimação quanto na interpretação dos parâmetros espaciais.
:::
...
hist.car <- histdens(queimadas.sem2$fitted.values, "Pred CAR")
...
# ------------------------------
# Resíduos - CAR
mapa_rj$brks.res.car <- cut(queimadas.sem2$residuals,
                             breaks = breaks_res,
                             labels = labels_res,
                             include.lowest = TRUE,
                             right = TRUE)
...
## Bõnus: Adicionando a conexão Rio-Niterói

### Aqui iremos adicionar manualmente a conexão Rio-Niterói

Aqui está o detalhe importante: Rio de Janeiro e Niterói não compartilham uma fronteira terrestre porque estão separados pela água (Baía de Guanabara). Mas em muitas análises, você pode querer tratá-los como vizinhos mesmo assim, porque:

- Estão muito próximos geograficamente
- Há grande fluxo de pessoas entre eles (trabalho, comércio)
- Compartilham características socioeconômicas similares
- Podem ter políticas integradas

O script vai adicionar essa conexão artificialmente, criando um "link" entre eles na matriz.
...
# ============================================================================
# BAIXANDO A MALHA MUNICIPAL DO ESTADO DO RIO DE JANEIRO
# ============================================================================
# code_muni = 33 (código do estado do RJ)
...
# ============================================================================
# 2. ADICIONAR CONEXÃO MANUAL ENTRE RIO DE JANEIRO E NITERÓI
# ============================================================================

# Primeiro, precisamos identificar os índices (posições) dos municípios de 
# Rio de Janeiro e Niterói no objeto mapa_rj.
# Assumindo que a coluna com os nomes dos municípios se chama "name_muni" 
# (ajuste o nome da coluna se for diferente no seu shapefile).
idx_rj <- which(mapa_rj$name_muni == "Rio de Janeiro")
idx_niteroi <- which(mapa_rj$name_muni == "Niterói")

# Adicionar conexão 
vizinhos_rainha[[idx_rj]] <- c(vizinhos_rainha[[idx_rj]], idx_niteroi)
vizinhos_rainha[[idx_niteroi]] <- c(vizinhos_rainha[[idx_niteroi]], idx_rj)
...
coord <- st_coordinates(st_centroid(st_geometry(mapa_rj)))

# Converte a lista de vizinhos espaciais em linhas e depois transforma em um objeto sf
viz.sf <- as(nb2lines(vizinhos_rainha, coords = coord), 'sf')

# Atribui ao objeto viz.sf o mesmo sistema de coordenadas (CRS) do mapa do Rio de Janeiro
viz.sf <- st_set_crs(viz.sf, st_crs(mapa_rj))

# ============================================================================
# 4. PLOTAR O GRAFO DE CONECTIVIDADE
# ============================================================================

# Figure 1: Mapa de vizinhança por conectividade com a ligação Rio-Niterói
mapa.viz <- ggplot(mapa_rj) + 
  geom_sf(fill = "pink", color = "#4A4A4A", linewidth = 0.3) +
  geom_sf(data = viz.sf, aes(color = "darkblue"), linewidth = 0.5, show.legend = FALSE) +
  scale_color_identity() +  # força a cor exata
  theme_minimal() +
  ggtitle("Vizinhança por conectividade (vizinhos_rainha)") +
  ylab("Latitude") +
  xlab("Longitude")
