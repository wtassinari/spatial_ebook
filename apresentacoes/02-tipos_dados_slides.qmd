...
plot(dados_pontos$latitude, dados_pontos$longitude, pch = 16, col = "red", main = "Focos de Queimadas")
```

<div style="font-size: 60%;">
Tentativa de plotar os pontos via um plano cartesiano.
</div>
...
- Sem um CRS, uma coordenada não tem significado geográfico. Por exemplo:
...
# Baixar polígono do RJ (código IBGE = 33)
rj_contorno <- read_state(year = 2020) |>
...
<div style="font-size: 70%;">
Acrescentando a escala e o norte
</div>
...
library(ggplot2)
library(sf)
library(ggspatial)  # Para a rosa dos ventos

# Definir a projeção adequada para o Rio de Janeiro
# UTM zona 23S (EPSG:31983) - recomendada para o RJ continental
# Ou South America Albers Equal Area (EPSG:5880)
projecao_rj <- st_crs(31983)  # UTM 23S - metros, boa para áreas e distâncias


# Plotar com projeção e rosa dos ventos
ggplot() +
  geom_sf(data = rj_contorno, fill = "lightgreen", color = "black") +
  geom_sf(data = pontos_sf, color = "red", size = 1.0, alpha = 0.7) +
  
  # Adicionar rosa dos ventos (north arrow)
  annotation_north_arrow(
    location = "tr",  # Posições: "tl" (topo esquerdo), "tr" (topo direito), 
                      # "bl" (base esquerda), "br" (base direita)
    which_north = "true",  # "true" = norte geográfico, "grid" = norte da grade
    pad_x = unit(0.2, "cm"),
    pad_y = unit(0.3, "cm"),
  ...
  # Adicionar barra de escala (opcional, mas recomendada)
  annotation_scale(
    location = "br",
    width_hint = 0.25,
    pad_x = unit(0.2, "cm"),
  ...
  labs(
    title = "Focos de Queimadas - Rio de Janeiro"
  ) +
