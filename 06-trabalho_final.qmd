...
- **Formato de entrega:** `.qmd` ou `.Rmd` com código reprodutível ou em `.pdf` com código e *outputs*
- **Prazo:** 22/06/2025 até 00:00
- **Envio:** Via tarefa cadastrada pelo SIGAA
...
Os alunos deverão trabalhar **individualmente** e a entrega será feita em formato **Quarto (.qmd)** ou **RMarkdown (.Rmd)** ou **PDF (.pdf)** até o dia **22/06 às 00:00**.
...
- **Mapas temáticos** mostrando a distribuição espacial da variável resposta;
- **Mapas temáticos** das variáveis explicativas;
...
- **Modelo 2: Regressão Espacial SEM (Spatial Error Model))**
  - Incorporação da dependência espacial no termo de erro;
  - Interpretação do coeficiente de autocorrelação espacial (λ).

- **Modelo 3: Regressão Geograficamente Ponderada (GWR)**
  - Interpretação dos coeficientes de regressão ao longo da área de estudo apenas das variáveis explicativas que julgar pertinentes a incorporação do modelo.
  - Plotar o mapa temátios dos coeficientes, dos  $R^2$ e dos resíduos do modelo.
...
1. **Reprodutibilidade:** Certifique-se de que todo o código está comentado e que alguém consegue reproduzir sua análise executando o arquivo `.qmd` ou `.rmd`. Se fizer em `.pdf`, é necessário que todo código utilizado esteja presente + os *outputs* .
...
O conceito final será baseado nos questionario respndidos pelo SIGAA + esse trabalho final

$$Conceito = \frac{(1 \times Q1) + (1 \times Q2) + (1 \times Q3) + (3 \times Trabalho)}{6}$$
::: 
